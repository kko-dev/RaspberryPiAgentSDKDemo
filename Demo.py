...
GPIO.setup(HELP_BUTTON, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 10 to be an input pin and set initial value to be pul
led low (off)
GPIO.add_event_detect(HELP_BUTTON,GPIO.RISING,callback=support_button_callback) # Setup event on pin 10 rising edge

GPIO.setup(APPROVE, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 10 to be an input pin and set initial value to be pulled 
low (off)
GPIO.add_event_detect(APPROVE,GPIO.RISING,callback=approve_button_callback) # Setup event on pin 10 rising edge

GPIO.setup(REJECT, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 10 to be an input pin and set initial value to be pulled l
ow (off)
GPIO.add_event_detect(REJECT,GPIO.RISING,callback=reject_button_callback) # Setup event on pin 10 rising edge

GPIO.setup(TERMINATE_ALL, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 10 to be an input pin and set initial value to be p
ulled low (off)
GPIO.add_event_detect(TERMINATE_ALL,GPIO.RISING,callback=lambda channel: terminate_button_callback(channel, tv_session_manag
ement)) # Setup event on pin 10 rising edge
